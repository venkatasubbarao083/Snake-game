...
public class Util {
    ...
    public static void log(final String event, WorkFlow workFlow) {
        ...
        if(event.equals(Constants.STEPSON)){
            logstepson(workFlow);
        }
        if(event.equals(Constants.STEPSON_MOVE)){
            logstepson_move(workFlow);
        }

    }

    private static void logstepson(WorkFlow workFlow) {
        if (workFlow.turnFirstPlayer) {
            System.out.println(workFlow.player1.name + " is steps on "+workFlow.player2.name
                    + workFlow.player1.position.x + ", " + workFlow.player1.position.y);
        }else{
            System.out.println(workFlow.player2.name + " is steps on "+workFlow.player1.name
                    + workFlow.player2.position.x + ", " + workFlow.player2.position.y);
        }
    }
    private static void logstepson_move(final WorkFlow workFlow){
        if(workFlow.turnFirstPlayer){
            System.out.println(workFlow.player2.name +" will now go to "+workFlow.player2.position.x+", "+workFlow.player2.position.y);
        }else {
            System.out.println(workFlow.player1.name +" will now go to "+workFlow.player1.position.x+", "+workFlow.player1.position.y);
        }
    }
    ...
    //snakes are created in the game
    ...
    public static void checkstepson(final Box[][] board,WorkFlow workFlow){
        if(((workFlow.player1.position.x==workFlow.player2.position.x)||(workFlow.player2.position.x==workFlow.player1.position.x))
                &&((workFlow.player1.position.y==workFlow.player2.position.y)||(workFlow.player2.position.y==workFlow.player1.position.y))){
            log(Constants.STEPSON,workFlow);
            if(workFlow.turnFirstPlayer){
            ...
            log(Constants.STEPSON_MOVE,workFlow);
        }
    }
